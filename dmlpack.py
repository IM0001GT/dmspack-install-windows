...
# ---------------------------------------------------------------------------
# Windows portability (Docker Desktop + native Windows clients)
# ---------------------------------------------------------------------------
...
def docker_path(p: Path | str) -> str:
    """Path form that Docker Desktop on Windows accepts for -v binds."""
    s = str(Path(p).resolve())
    if is_windows():
        # Docker Desktop accepts C:\foo and //c/foo; prefer resolved Windows path.
        return s
    return s


def extract_zstd_tar(archive: Path, dest: Path) -> None:
    """Decompress a .tar.zst member into dest. Uses host zstd+tar, else Docker alpine."""
    dest.mkdir(parents=True, exist_ok=True)
    zstd = shutil.which("zstd") or shutil.which("zstd.exe")
    tar = shutil.which("tar") or shutil.which("tar.exe")
    if zstd and tar:
        p1 = subprocess.Popen([zstd, "-d", "-c", "-q", str(archive)], stdout=subprocess.PIPE)
        p2 = subprocess.Popen([tar, "-xf", "-", "-C", str(dest)], stdin=p1.stdout)
        assert p1.stdout is not None
        p1.stdout.close()
        if p2.wait() != 0 or p1.wait() != 0:
            raise SystemExit(f"{archive.name}: host zstd|tar extract failed")
        return

    print_info("host zstd not found -- extracting via Docker alpine (works on Windows)")
    if run(["docker", "info"], check=False, capture=True).returncode != 0:
        raise SystemExit("need either zstd on PATH or a running Docker daemon to extract .tar.zst")
    src_dir = archive.parent.resolve()
    dest_dir = dest.resolve()
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{docker_path(src_dir)}:/in:ro",
        "-v", f"{docker_path(dest_dir)}:/out",
        "alpine:3.20",
        "sh", "-c",
        f"apk add --no-cache zstd >/dev/null && zstd -d -c -q /in/{archive.name} | tar -xf - -C /out",
    ]
    r = run(cmd, check=False, capture=True)
    if r.returncode != 0:
        print_error((r.stderr or r.stdout or "").strip()[:500])
        raise SystemExit(f"{archive.name}: docker zstd extract failed")
...
def default_archive_dir() -> Path:
    """Pick where archives should live.

    On a Steam Deck that is the SD card (automounted under /run/media/deck). On
    any other machine it is the first writable mount under the common removable
    mount points that is not the home filesystem, falling back to ~/.dmlpack.
    Pass --out to override.
    """
    seen: set[Path] = set()
    for root in ("/run/media", "/media", "/mnt"):
        for cand in sorted(Path(root).glob("*/")) + sorted(Path(root).glob("*/*/")):
            cand = cand.resolve()
            if cand in seen or not cand.is_dir() or not os.access(cand, os.W_OK):
                continue
            seen.add(cand)
            if not same_filesystem(cand, Path.home()):
                return cand / "dmlpack"
    home = Path.home() / ".dmlpack"
    print_warning("no writable removable mount found -- using ~/.dmlpack")
    return home
...
def docker_uid_args() -> list[str]:
    ...
    # Windows Docker Desktop has no meaningful host uid mapping for this purpose.
...
def restore_root_tar_zst(m: dict, blob: Path) -> None:
    ...
    if zstd:
        ...
        r = run([
            "docker", "run", "--rm",
            "-v", f"{docker_path(blob_dir)}:/in:ro",
            "-v", f"{docker_path(base)}:/v",
            "alpine:3.20",
            "sh", "-c",
            f"apk add --no-cache zstd >/dev/null && zstd -d -c -q /in/{blob.name} "
            f"| tar -C /v --numeric-owner -xf -",
        ], check=False, capture=True)
        if r.returncode != 0:
            print_error((r.stderr or r.stdout or "").strip()[:500])
            raise SystemExit(f"{m['path']}: root tar import failed")
...
def restore_docker_volume(m: dict, blob: Path, manifest: dict | None = None) -> None:
    ...
    if zstd:
        ...
        # Fully inside Docker so Windows hosts without zstd still restore.
        blob_dir = blob.parent.resolve()
        r = run([
            "docker", "run", "--rm",
            "-v", f"{docker_path(blob_dir)}:/in:ro",
            ...
            "alpine:3.20",
            "sh", "-c",
            f"apk add --no-cache zstd >/dev/null && zstd -d -c -q /in/{blob.name} | tar -C /v -xf -",
        ], check=False, capture=True)
        if r.returncode != 0:
            print_error((r.stderr or r.stdout or "").strip()[:500])
            raise SystemExit(f"{m['path']}: volume import failed")
...
def restore_docker_image(m: dict, blob: Path) -> None:
    ...
    if zstd:
        ...
        blob_dir = blob.parent.resolve()
        # Decompress to a temp tar inside the same folder, then docker load.
        tmp_tar = blob_dir / (blob.name + ".load.tar")
        r = run([
            "docker", "run", "--rm",
            "-v", f"{docker_path(blob_dir)}:/in",
            "alpine:3.20",
            "sh", "-c",
            f"apk add --no-cache zstd >/dev/null && zstd -d -c -q /in/{blob.name} > /in/{tmp_tar.name}",
        ], check=False, capture=True)
        if r.returncode != 0:
            print_error((r.stderr or r.stdout or "").strip()[:500])
            raise SystemExit(f"{m['path']}: image decompress failed")
        with tmp_tar.open("rb") as fh:
            p2 = subprocess.Popen(["docker", "load"], stdin=fh)
            if p2.wait() != 0:
...
def preflight_pack(spec: dict, outdir: Path) -> int:
    ...
    if same_filesystem(outdir, Path.home()):
        problems.append(f"destination {contract(outdir)} is on the same filesystem as /home. "
                        "Archives should go to removable storage -- /home has no room to "
                        "stage a copy (or pass a different --out).")
...
def port_holders(ports: list[int]) -> dict[int, str]:
    ...
    if is_windows():
        # Prefer PowerShell Get-NetTCPConnection; fall back to netstat.
        ps = (
            "Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue "
            "| Select-Object -ExpandProperty LocalPort"
        )
        r = run(["powershell", "-NoProfile", "-Command", ps], check=False, capture=True)
        if r.returncode == 0 and r.stdout:
            listening = {int(x.strip()) for x in r.stdout.splitlines() if x.strip().isdigit()}
            for port in ports:
                if port in listening:
                    held[port] = "listening process (Windows)"
            return held
        r = run(["netstat", "-ano"], check=False, capture=True)
        if r.returncode != 0:
            return held
        for line in r.stdout.splitlines():
            ...
            port = int(mm.group(1))
            if port in ports:
                held[port] = "listening process (netstat)"
        return held
...
def cmd_restore(args) -> int:
        ...
        if m["kind"] == "qcow2":
            ...
            if m.get("if_absent_only"):
                ...
                if present:
                    ...
                    continue
            dest.mkdir(parents=True, exist_ok=True)
            extract_zstd_tar(scratch, dest)
            print_success(f"{contract(dest)}")
            # The server dir is packed on a Deck; retarget any /home/deck it
            # bakes in. No-op on a Deck, skips binaries, and deliberately skips
            # the client tar (a huge wine prefix where the rewrite is both
            # unlikely to match and slow).
            if m.get("kind") == "tar_zst" and "client" not in m.get("path", ""):
                rewrite_deck_paths_in_dir(dest)
            ...
            extract_zstd_tar(scratch, unpacked)
    ...
    # 4. steam shortcuts (+ Windows desktop/start launchers)
    shortcuts_ok = True
    if recipe.get("steam", {}).get("shortcuts"):
        if is_windows():
            shortcuts_ok = register_shortcuts_windows(manifest, recipe["steam"])
        else:
            shortcuts_ok = register_shortcuts(recipe["steam"])

    # 4b. Always install native Windows PowerShell launchers when on Windows
    if is_windows():
        install_windows_launchers(manifest)
    ...
    if prefix.get("regenerate") and not is_windows():
    ...
    if is_windows() and prefix.get("regenerate"):
        print_info("Windows: client runs native .exe (no Proton prefix needed).")
    ...
    if not shortcuts_ok:
        ...
        if is_windows():
            print_info("Desktop / DML-Launchers shortcuts were still installed when possible.")
        return 1
    if is_windows():
        print_info("Use the Desktop shortcuts or %USERPROFILE%\\DML-Launchers to start.")
        print_info("Start the SERVER first, wait until READY, then start the game.")
    else:
...
def install_windows_launchers(manifest: dict) -> None:
    """Install PowerShell server/client/stop scripts + Desktop shortcuts for Windows."""
    game = manifest.get("game", "")
    display = manifest.get("display_name", game)
    print_step(f"installing Windows launchers for {display}")
    out = windows_launcher_dir()
    kit_launchers = SELF_DIR / "launchers"
    home = Path.home()

    mapping = {
        "evejs": [
            ("Evejs-Server.ps1", "EVE Online Server.lnk"),
            ("Evejs-Client.ps1", "EVE Online.lnk"),
            ("Evejs-Stop.ps1", "EVE Server Stop.lnk"),
        ],
        "tortoise-wow-v2": [
            ("TurtleWow-Server.ps1", "Turtle WoW V2 Server.lnk"),
            ("TurtleWow-Client.ps1", "Turtle WoW.lnk"),
            ("TurtleWow-Stop.ps1", "Turtle WoW V2 Stop.lnk"),
        ],
        "tortoise-wow": [
            ("TurtleWow-Server.ps1", "Turtle WoW Server.lnk"),
            ("TurtleWow-Client.ps1", "Turtle WoW.lnk"),
            ("TurtleWow-Stop.ps1", "Turtle WoW Stop.lnk"),
        ],
    }
    files = mapping.get(game)
    if not files:
        print_warning(f"no built-in Windows launchers for game id {game!r}")
        print_info(f"Server files restored under {home}; start them with docker compose manually.")
        return
    ...
    for src_name, lnk_name in files:
        src = kit_launchers / src_name
        if not src.is_file():
        ...
        if desktop.is_dir():
            try:
                ...
                print_success(f"Desktop: {lnk_name}")
            except Exception as exc:  # noqa: BLE001
                print_warning(f"could not create desktop shortcut {lnk_name}: {exc}")

    print_info(f"Launchers directory: {out}")


def create_windows_shortcut(lnk_path: Path, target: str, arguments: str = "",
                            workdir: str = "", description: str = "") -> None:
    """Create a .lnk via PowerShell WScript.Shell (no pywin32 required)."""
    lnk_path = Path(lnk_path)
    lnk_path.parent.mkdir(parents=True, exist_ok=True)
    ps = f"""
$w = New-Object -ComObject WScript.Shell
$s = $w.CreateShortcut('{str(lnk_path).replace("'", "''")}')
...
def register_shortcuts_windows(manifest: dict, steam: dict) -> bool:
    """Windows Steam + desktop shortcut registration.

    Prefer native PowerShell launchers over konsole-based Deck shortcuts.
    Still writes Steam shortcuts.vdf when Steam is installed and closed.
    """
    install_windows_launchers(manifest)

    # Remap Deck server shortcuts to PowerShell launchers when writing Steam VDF
    game = manifest.get("game", "")
    launcher_dir = windows_launcher_dir()
    ps = shutil.which("powershell.exe") or r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"
    remapped = []
    for sc in steam.get("shortcuts", []):
        sc = dict(sc)
        name = sc.get("name", "")
        script = None
        if game == "evejs":
            if "Server Stop" in name or name.endswith("Stop"):
                script = launcher_dir / "Evejs-Stop.ps1"
            elif "Server" in name:
                script = launcher_dir / "Evejs-Server.ps1"
            elif name in ("EVE Online", "EVE"):
                script = launcher_dir / "Evejs-Client.ps1"
        elif game.startswith("tortoise-wow"):
            if "Stop" in name:
                script = launcher_dir / "TurtleWow-Stop.ps1"
            elif "Server" in name:
                script = launcher_dir / "TurtleWow-Server.ps1"
            else:
                script = launcher_dir / "TurtleWow-Client.ps1"
    ...
    steam_win = dict(steam)
    steam_win["shortcuts"] = remapped
    # On Windows clients, do not force Proton compat mappings for .exe shortcuts
    # that we intentionally remapped to powershell; native .exe client shortcuts
    # that remain still get proton mapping only if they end with .exe — leave default.
    return register_shortcuts(steam_win)
...
def place_extras(m: dict, unpacked: Path) -> None:
    ...
    for f in m["files"]:
        if "glob" in f:
            ...
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(src, dst)
            if f.get("mode") and not is_windows():
                try:
                    os.chmod(dst, int(f["mode"], 8))
                except OSError:
                    pass
            portabilize_file(dst)
...
def steam_is_running() -> bool:
    if is_windows():
        r = run(["tasklist", "/FI", "IMAGENAME eq steam.exe"], check=False, capture=True)
        if r.returncode == 0 and r.stdout and "steam.exe" in r.stdout.lower():
            return True
        r = run(["tasklist", "/FI", "IMAGENAME eq steamwebhelper.exe"], check=False, capture=True)
        return r.returncode == 0 and r.stdout is not None and "steamwebhelper.exe" in r.stdout.lower()
...
def steam_roots() -> list[Path]:
    if is_windows():
        ...
        for env in env_candidates:
            if env:
                roots.append(Path(env) / "userdata")
        pf86 = os.environ.get("PROGRAMFILES(X86)") or r"C:\Program Files (x86)"
        pf = os.environ.get("PROGRAMFILES") or r"C:\Program Files"
        roots += [
            Path(pf86) / "Steam" / "userdata",
            Path(pf) / "Steam" / "userdata",
...
def global_config_vdf() -> Path | None:
    """The Steam-wide config/config.vdf (CompatToolMapping lives here)."""
    for root in steam_roots():
        cand = root.parent / "config" / "config.vdf"
        if cand.exists():
            return cand
    for root in steam_roots():
        if root.is_dir():
            return root.parent / "config" / "config.vdf"
    return None
...
def _match_brace(text: str, open_i: int) -> int:
    ...
    for i in range(open_i, len(text)):
        if text[i] == "{":
            ...
            if depth == 0:
                return i
    raise ValueError("unbalanced braces in vdf")


def apply_compat_mappings(mappings: dict[int, str], vdf_path: Path) -> bool:
    """Write Steam-play entries into CompatToolMapping for non-Steam shortcuts.

    Windows binaries (the client) die instantly under Steam without a
    compatibility tool -- the manifest cannot carry one, so we set it here.
    Keys are the unsigned appid, matching what Steam's launcher uses.
    """
    if not mappings:
        return True
    if vdf_path.exists():
        text = vdf_path.read_text(encoding="utf-8", errors="surrogateescape")
        backup = vdf_path.with_name(f"config.vdf.backup-{time.strftime('%Y%m%d-%H%M%S')}")
    ...
    idx = text.find('"CompatToolMapping"')
    if idx == -1:
        if text.strip() and not text.rstrip().endswith("}"):
            print_warning("config.vdf has no root level; not touching compat settings")
            return False
        entry_indent = "\t" * 5
        block = '"CompatToolMapping"\n{\n' + "".join(
            _compat_entry(k, v, entry_indent) for k, v in mappings.items()
        ) + "\t}\n"
        ...
        head, block, tail = text[:open_i + 1], text[open_i + 1:close_i], text[close_i:]
        m = re.search(r"(?m)^([ \t]*)\"\d+\"", block)
        entry_indent = m.group(1) if m else "\t" * 5
        if any(re.search(rf"(?m)^{re.escape(entry_indent)}\"{k}\"$", block) for k in mappings):
            print_info("some compat mappings already present -- leaving those alone")
        for k, v in mappings.items():
            if not re.search(rf"(?m)^{re.escape(entry_indent)}\"{k}\"$", block):
                block = block.rstrip() + "\n" + _compat_entry(k, v, entry_indent)
        text = head + block + tail

    vdf_path.write_text(text, encoding="utf-8", errors="surrogateescape")
    print_success(f"{contract(vdf_path)} -- {len(mappings)} compat tool mapping(s)")
    return True


def _compat_entry(appid: int, tool: str, indent: str) -> str:
    inner = indent + "\t"
...
# ------------------------------------------------------------------ portability
# Packs are made on a Steam Deck where the home is /home/deck and the default
# terminal is konsole. A restore must adapt: rewrite a hard-baked /home/deck to
# THIS machine's home, and swap a missing terminal for one that is installed.
# Both rewrites are no-ops on a Deck, so a restored pack behaves identically
# there -- they only change anything on a machine that is not a Deck.

TERMINALS = ("konsole", "kitty", "alacritty", "wezterm", "ghostty",
             "foot", "kgx", "gnome-terminal", "tilix", "xterm")


def portabilize(s: str) -> str:
    """Rewrite a packed path that bakes in the Deck's home onto this machine."""
    if not s:
        return s
    home = str(Path.home())
    # Prefer forward-slash home for scripts that still look Unix-like; keep
    # native separators for Windows .exe shortcut paths.
    unix_home = home.replace("\\", "/")
    s = s.replace("/home/deck", unix_home if not is_windows() else unix_home)
    if is_windows():
        s = s.replace("\\home\\deck", home)
        # Many Deck shortcuts bake absolute Linux terminal paths.
        s = s.replace("/usr/bin/konsole", "powershell.exe")
        s = s.replace("/usr/bin/kitty", "powershell.exe")
    return s


def terminal_for(exe: str) -> str:
    """If exe names a terminal emulator that is missing here, return the first
    installed one (preserving quoting). Otherwise return exe unchanged."""
    raw = exe.strip()
    if not raw:
        return exe
    if is_windows():
        name = Path(raw.strip('"')).name.lower()
        if name in {t.lower() for t in TERMINALS} or name in ("konsole", "bash", "sh"):
            # Server launchers on Windows use PowerShell scripts, not konsole.
            return "powershell.exe"
        return exe
    quoted_exe = raw.startswith('"') and raw.endswith('"')
    ...
    for t in TERMINALS:
        if shutil.which(t):
            return f'"/usr/bin/{t}"' if quoted_exe else t
    return exe


def portabilize_entry(entry: dict) -> dict:
    """Apply home + terminal portability to a captured shortcut entry."""
    out = dict(entry)
    for k in ("Exe", "StartDir", "LaunchOptions"):
        if k in out:
            out[k] = portabilize(out[k])
    if "Exe" in out:
        out["Exe"] = terminal_for(out["Exe"])
    return out


def portabilize_file(p: Path) -> int:
    """Rewrite hard-baked /home/deck paths to this machine's home in a restored
    text file. Skips binaries and big files. No-op on a Deck. Returns 1 if changed."""
    if not p.is_file():
        return 0
    try:
        ...
        return 0
    if "/home/deck" not in text:
        return 0
    p.write_text(text.replace("/home/deck", str(Path.home())), errors="replace")
    return 1


def rewrite_deck_paths_in_dir(root: Path) -> int:
    """Rewrite /home/deck -> $HOME across a restored tree (server dir etc.)."""
    n = 0
    for p in root.rglob("*"):
        if p.is_file():
            n += portabilize_file(p)
    if n:
        print_info(f"retargeted {n} file(s) under {contract(root)} to this machine's home")
    return n
...
def register_shortcuts(steam: dict) -> bool:
    ...
    for sc in steam["shortcuts"]:
        exe = terminal_for(portabilize(sc["exe"]))
        startdir = portabilize(sc.get("startdir") or "")
        launchopts = portabilize(sc.get("launchopts") or "")
        qexe, qdir = quoted(exe), quoted(startdir or str(Path(exe).parent))
        if any((e.get("AppName") or e.get("appname")) == sc["name"] for e in shortcuts.values()):
            ...
            continue
        if sc.get("vdf"):
            # Replay the entry that was live when this was packed, adapted to
            # this machine (home path + terminal). No-op on a Deck.
            entry = portabilize_entry(dict(sc["vdf"]))
            effective_exe = entry.get("Exe", qexe)
            appid = gen_appid(effective_exe, sc["name"])
            stored = (entry.get("appid") if "appid" in entry else to_signed(appid)) & 0xFFFFFFFF
            source = "captured"
        else:
            effective_exe = qexe
            appid = gen_appid(qexe, sc["name"])
            stored = to_signed(appid) & 0xFFFFFFFF
            entry = {
        ...
        if effective_exe.strip('"').lower().endswith(".exe"):
            mappings[stored] = sc.get("compat_tool") or steam.get("compat_tool") or "proton_experimental"
        shortcuts[str(max((int(k) for k in shortcuts), default=-1) + 1)] = entry
        added += 1
        print_success(f"{sc['name']}  appid {stored} ({source}; grid art: {stored}*.png)")
    ...
    if mappings:
        cfg_vdf = global_config_vdf()
        if cfg_vdf is None:
            print_warning("could not locate Steam's config/config.vdf -- compat tool not set")
        else:
            apply_compat_mappings(mappings, cfg_vdf)
    return True
