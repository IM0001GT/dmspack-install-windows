...
| Server/client shortcut fails with parse errors / "misaligned character" / garbled `â€"` in `Evejs-Server.ps1` | Fixed in **v1.1.4**: launcher templates used Unicode em-dashes written as UTF-8 **without BOM**. Windows PowerShell 5.1 then misread them and broke string literals. Launchers are now ASCII-only and installed with UTF-8 BOM. **Already installed?** Menu **4) Shortcuts** (or re-run install) refreshes `%USERPROFILE%\DML-Launchers\*.ps1`. |
